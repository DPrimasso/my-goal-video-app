...
const PlayerVisual: React.FC<{
    ...
    zIndex: number;
    blur: number
}> = ({player, x, y, imageScale, nameScale, nameMarginTop, zIndex, blur}) => {
    ...
    return (
        <>
            {validPlayersWithPositions.map(({player, originalIndex}, i) => {
                ...
                return <PlayerVisual key={`${originalIndex}-${player.name}`} player={player} x={x} y={y}
                                     imageScale={imageScale} nameScale={nameScale} nameMarginTop={nameMarginTop}
                                     zIndex={zIndex} blur={playerBlur}/>;
            })}
        </>
    );
};
    ...
    if (attackingMidfielders.some(player => player !== null)) {
        sequences.push(
            <Sequence from={current} key="attackingMidfielders">
                <GroupIntro players={attackingMidfielders} finalPositions={positions.attackingMidfielders}
                            duration={groupDuration} finalBlur={blurIntensity}/>
            </Sequence>
        );
        current += groupDuration;
    }
